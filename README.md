...
**Is it free?** The CLI and local proxy are **free forever** (open source, Apache-2.0, self-host, no seat limits, no time limit). That includes `tokenfuse mcp-scan` and its GitHub Action. The hosted **Cloud** has a **free observe tier**: full live fleet visibility (burn rate, per-run spend vs. cap, near-cap alerts). *Acting* on the fleet from the dashboard (the kill-switch and central budgets) and the advanced surface (savings, incidents, audit, compliance, mobile push) are the **paid** plan, billed at a **flat monthly rate for unlimited seats**, not per-seat or a percentage of spend. Self-hosting the Cloud with your own keys stays unrestricted. Exact pricing isn't finalized yet.
